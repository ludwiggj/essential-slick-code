package chapter05.representations.rows.tuples
...
trait DatabaseModule3 {
  ...
  final class MessageTable(tag: Tag) extends Table[(String, String, Long)](tag, "message") {
...
object DbAbstractionSimple extends App {
  ...
  val databaseLayer = new DatabaseModule3 {
  ...
  ).map(msg => (msg.sender, msg.content, msg.id))
