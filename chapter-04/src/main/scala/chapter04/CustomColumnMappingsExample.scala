...
object CustomColumnMappingsExample extends App {
  ...
  trait Tables {
    ...
    def numSenders = messages.map(_.senderId).countDistinct
  }
  ...
  db.withSession {
    implicit session =>
      ...
      val q = for {
        msg <- messages
        usr <- msg.sender
      } yield (usr.name, msg.content, msg.ts)
      ...
      // Cascade delete
      println("Rows deleted: " + users.filter(_.name === "Dave").delete)
